...
状态：v0 已实现。目录站每天两次走五种引擎；Firecrawl 只用于主题 Search 和引擎抽不出正文时的 Scrape。整站 Crawl、发现源自动加入定时采集、个人日报投递仍不在范围内。
...
| Scrape | 五种目录引擎抽不出正文时才抓单页 | 只请求 markdown 与 metadata；每页 1 credit |
...
5. 发现到的 URL 先走五种引擎抽正文；只有引擎失败或不稳定时才 Scrape。不要把 Firecrawl 当目录站的每日爬虫。
